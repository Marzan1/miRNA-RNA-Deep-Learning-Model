...
def prepare_dataset():
    ...
    skipped_no_affinity_pair_count = 0 # This count should now be very low, as we filtered upfront
        ...
        for rre_idx, (rre_id, current_rre_seq) in enumerate(all_rre_sequences):
            ...
            for region_name, region_seq in current_target_regions.items():
                if validate_seed(mirna_seq, region_seq):
                    ...
                    # affinity.get(mirna_id, None) will now very rarely be None due to pre-filtering
                    mirna_affinity_score = affinity.get(mirna_id) # No default needed, as it should exist now
                    # We still check for None here just in case, but skipped_no_affinity_pair_count should be low
